...
两层架构：第一层双阈值检测扰流存在；第二层 D0 标准化 → PCA 降维 → Mahalanobis 距离 A/B 分类。留一日期交叉验证中，每折以训练集 `profile_top_bottom` 均值自动对齐 A/B 标签方向（避免无监督聚类标签在折间翻转）。折外 A/B 二分类正确率 149/149（100%）。`profile_top_bottom` 的正负方向在不同日期间高度稳定，因此该分类具有跨日期泛化能力。
