...
问题2采用 OWICS——模型权重由 Lagrange 插值积分推导（与附件7偏差 < $10^{-5}$），不含数据拟合参数，符合无扰流问题定位。问题4将引入 Phys6 作为残差补偿基线——Phys6 在全量扰流数据上 MAE 仅 $0.181\%$（OWICS 为 $0.506\%$），更适合在复杂流场下搭配非线性补偿。
...
12 维特征（5 归一化 chord + 5 AB 差异 + profile_swirl + profile_ab_abs），D0 参照 Z-score 标准化，PCA 降至 2 维（方差 94.6%），Ward 层次聚类。K=2 时轮廓系数 0.617，高于 K=3（0.532）、K=4（0.491）、K=5（0.473）。A 类 = {D1, D2, D5, D7}，B 类 = {D3, D4, D6, D8}。两类核心差异为 `profile_top_bottom` 正负：A 类表现为上部声道相对增强，B 类表现为下部声道相对增强。
...
ET(28d) 在 $u_r$（0.114\%）和 $u_d$（0.280\%）上最低，但 $u_L$（0.203\%）高于ET(13d)（0.189\%）。13 维简化模型（profile+dyn+rate）同为 10/30，可作为论文简化对照。
